...
class IdentityCacheConfig {
    ...
    @Bean
    fun refreshTokenRotationScript(cacheProperties: IdentityCacheProperties): DefaultRedisScript<String> = DefaultRedisScript<String>().apply {
        resultType = String::class.java
        val deleteCommand = if (cacheProperties.unlinkEnabled) "UNLINK" else "DEL"
        setScriptText(
            """
            ...
            redis.call('$deleteCommand', KEYS[2])
            return 'ROTATED'
            """.trimIndent()
        )
    }
